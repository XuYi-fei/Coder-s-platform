...
-- =====================================================
-- 预置技能（skill_config）
-- skill 是一个有结构的文件夹，核心是 SKILL.md。
-- SKILL.md = YAML frontmatter（name/description 等元数据）+ Markdown 正文（指令）
-- skill_type:
--   SIMPLE — 仅含 SKILL.md，无额外资源文件；可在线创建或通过只含 SKILL.md 的 zip 上传
--   FOLDER — 含 SKILL.md + 额外资源目录（scripts/、references/、assets/）；需上传 zip
-- =====================================================

INSERT IGNORE INTO `skill_config`
  (`name`, `display_name`, `description`, `skill_type`, `content`, `skill_md`, `tags`, `visibility`, `status`, `is_system`, `version`, `creator_user_id`, `workspace_id`)
VALUES
-- 1. 编码规范审查
(
  'code_review',
...
name: code_review
display_name: 编码规范审查
description: 专业的代码审查助手，分析代码质量、发现潜在问题并给出改进建议
---
...
name: sql_assistant
display_name: SQL 助手
description: 专业的 SQL 查询助手，帮助编写、优化和解释 SQL 语句
---
...
display_name: 技术文档撰写
description: 协助撰写高质量的技术文档，包括 API 文档、设计文档、README 等
---
...
name: debug_helper
display_name: 代码调试助手
description: 帮助分析错误信息、定位 Bug 根因并给出修复方案
---
...
name: data_analyst
display_name: 数据分析助手
description: 协助进行数据分析、数据解读和可视化建议
---
